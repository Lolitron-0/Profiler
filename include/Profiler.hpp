...
namespace profiler
{
#define PROF_ASSERT(cond, message) if (!(cond)) {std::cerr << (std::string{"Assertion failed! "}+(message)); __debugbreak();}
    ...
    protected:
        explicit Singleton()
        {
            PROF_ASSERT(!Singleton::s_Instance, "Singleton instance already exists");
    ...
    struct ProfileResult
    {
        ...
        ProfileResult(const std::string& Name, const std::chrono::duration<double, std::micro>& Start, const std::chrono::microseconds& Elapsed, const std::thread::id& ThreadId)
            : Name(Name), Start(Start), Elapsed(Elapsed), ThreadId(ThreadId)
        {
        }
    };
    ...
    public:
        ...
        void BeginSession(const std::string& name, const std::string& filePath = "result.json")
        {
            ...
            if (m_CurrentSession)
            {
                ...
                PROF_ASSERT(false, "Session already opened: {0}!" + m_CurrentSession->Name);
                EndSessionNoLock_();
            }
            m_OutStream.open(filePath);

            PROF_ASSERT(m_OutStream.is_open(), "Could not open file: {0}!" + filePath);
        ...
        void WriteProfile(const ProfileResult& result)
        {
            ...
            PROF_ASSERT(m_CurrentSession, "No opened session!");
    ...
    private:
        void Stop()
        {
            ...
            auto highResStart{ std::chrono::duration<double, std::micro>{ m_Start.time_since_epoch() } };
            ...
            Profiler::GetInstance().WriteProfile({ m_Name, highResStart, elapsed, std::this_thread::get_id() });
        }
    private:
        std::string m_Name;
...
#define PROFILER_BEGIN_SESSION(name, filename) ::profiler::Profiler::GetInstance().BeginSession((name), (filename));
...
#define PROFILER_SCOPE_LINE2(name, line) ::profiler::ProfilerScope scope##line{ (name) };
#else
